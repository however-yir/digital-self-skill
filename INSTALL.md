...
然后在 Claude Code 中说 `/create-digital-self` 即可启动。

生成的分身 Skill 默认写入 `./selves/` 目录。
...
重启 OpenClaw session，说 `/create-digital-self` 启动。
...
2. 选择 **From scratch**
3. 填写 App Name（如 `digital-self-skill-bot`），选择目标 Workspace → **Create App**
...
> 提示：如果你不知道目标分身在哪些频道，可以先不邀请，运行采集时脚本会告知 Bot 加入了哪些频道，再补充邀请。
...
按提示粘贴 Bot Token，脚本会自动验证并保存到 `~/.digital-self-skill/slack_config.json`。

配置成功后你会看到：
```
验证 Token ... OK
  Workspace：Your Company，Bot：digital-self-skill-bot

✅ 配置已保存到 /Users/you/.digital-self-skill/slack_config.json
```
...
```bash
cd ~/.claude/skills/create-digital-self   # 或你的项目 .claude/skills/create-digital-self
...
```
digital-self-skill/        ← clone 到 .claude/skills/create-digital-self/
├── SKILL.md            # skill 入口（官方 frontmatter）
├── prompts/            # 分析和生成的 Prompt 模板
├── tools/              # Python 工具脚本
├── docs/               # 文档（PRD 等）
│
└── selves/         # 生成的分身 Skill 存放处（.gitignore 排除）
    └── {slug}/
